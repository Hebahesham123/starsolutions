...
const items = () => getContent(locale).automations.filter((entry) => !entry.hidden);
...
export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  ...
  const inEn = getContent('en').automations.some((entry) => entry.slug === params.slug);
  return { alternates: alternates('ar', `/automations/${params.slug}`, { en: inEn }), title: item?.title ??  t('page.automations.title'), description: item?.summary };
}

export default function AutomationsDetailPage({ params }: { params: { slug: string } }) {
  const list = items();
