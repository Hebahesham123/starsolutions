...
export const metadata: Metadata = {
  alternates: alternates('ar', '/automations'),
  title: t('page.automations.title'),
  description: t('page.automations.lede'),
};
...
export default function AutomationsIndexPage() {
  ...
  return (
    <>
      <PageHead
        eyebrow={t('page.automations.eyebrow')}
        title={t('page.automations.title')}
        lede={t('page.automations.lede')}
        crumbs={[{ href: localeHref(locale, '/'), label: t('crumbs.home') }, { label: t('page.automations.crumb') }]}
      />
          ...
          <div className="index-grid">
            {items.map((item, i) => (
              <Reveal as="article" key={item.id} delay={i * 0.05}>
                <Link
                  href={localeHref(locale, `/automations/${item.slug}`)}
